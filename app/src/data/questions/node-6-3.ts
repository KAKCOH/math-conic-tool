...
const questions6_3: (Question | ChoiceQuestion)[] = [
  ...
  // ===== 难度 2：切线放缩应用 =====
  {
    ...
    difficulty: 2,
    type: 'proof',
    content: '(1) 证明：对任意实数 $x$，有 $e^x \\ge x + 1$（等号仅当 $x = 0$ 时成立）；\n(2) 利用 (1) 证明：当 $x \\ge 0$ 时，$e^x \\ge 1 + x + \\frac{x^2}{2}$。',
    answer: '证明见解答。',
    solution: '(1) 令 $h(x) = e^x - x - 1$。$h\'(x) = e^x - 1$。\n当 $x < 0$ 时 $h\'(x) < 0$，$h(x)$ 单调递减；当 $x > 0$ 时 $h\'(x) > 0$，$h(x)$ 单调递增。$h(x)$ 的最小值为 $h(0) = e^0 - 0 - 1 = 0$，故 $h(x) \\ge 0$ 即 $e^x \\ge x + 1$。等号仅当 $x = 0$。\n\n(2) 由 (1) 知对任意 $t \\in \\mathbb{R}$ 有 $e^t \\ge t + 1$。对 $t \\in [0, x]$（$x \\ge 0$），$e^t \\ge t + 1$ 恒成立。两边在 $[0, x]$ 上积分：\n$$\\int_0^x e^t\\,dt \\ge \\int_0^x (t + 1)\\,dt$$\n$$e^x - 1 \\ge \\frac{x^2}{2} + x$$\n移项得 $e^x \\ge 1 + x + \\frac{x^2}{2}$，对一切 $x \\ge 0$ 成立。',
    tags: ['切线放缩', '积分法', '指数不等式'],
    source: '自主命题',
  },
  ...
  // ===== 难度 3：放缩法综合 =====
  {
    ...
    difficulty: 3,
    type: 'proof',
    content: '(1) 分别证明两条经典切线不等式：\n  \\quad ① $e^x \\ge x + 1$（$x \\in \\mathbb{R}$），等号仅当 $x = 0$；\n  \\quad ② $\\ln x \\le x - 1$（$x > 0$），等号仅当 $x = 1$。\n(2) 利用 (1) 中的不等式，证明：对任意 $x > 0$，有 $e^{x-1} \\ge x \\ge 1 + \\ln x$，并说明等号同时成立的条件。\n(3) 进一步证明：对任意 $x > 0$，有 $e^x - \\ln x > 2$。',
    answer: '证明见解答。',
    solution: '(1) ① 令 $h_1(x) = e^x - x - 1$，$h_1\'(x) = e^x - 1$。$x < 0$ 时 $h_1\' < 0$，$x > 0$ 时 $h_1\' > 0$，$h_1(x) \\ge h_1(0) = 0$，故 $e^x \\ge x + 1$，等号仅当 $x = 0$。\n② 令 $h_2(x) = x - 1 - \\ln x$（$x > 0$），$h_2\'(x) = 1 - \\frac{1}{x}$。$0 < x < 1$ 时 $h_2\' < 0$，$x > 1$ 时 $h_2\' > 0$，$h_2(x) \\ge h_2(1) = 0$，故 $\\ln x \\le x - 1$，等号仅当 $x = 1$。\n\n(2) 在 ① 中将 $x$ 替换为 $x-1$：$e^{x-1} \\ge (x-1) + 1 = x$，得 $e^{x-1} \\ge x$，等号仅当 $x-1 = 0$ 即 $x = 1$。\n② 即 $\\ln x \\le x - 1$，移项得 $x \\ge 1 + \\ln x$，等号同样仅当 $x = 1$。\n两式合并：$e^{x-1} \\ge x \\ge 1 + \\ln x$。等号在 $x = 1$ 时同时成立（左右两端均取等）。\n\n(3) 由 ①：$e^x \\ge x + 1$。由 ② 变形：$-\\ln x \\ge 1 - x$。两式相加得：$e^x - \\ln x \\ge (x + 1) + (1 - x) = 2$。\n若等号成立，需 $e^x = x + 1$（即 $x = 0$）且 $\\ln x = x - 1$（即 $x = 1$）同时成立，但 $x = 0$ 不在 $\\ln x$ 的定义域内，且 $\\ln 1 \\neq 1 - 1$ 不成立——实际上 $x=0$ 不可能同时满足两条等号条件。\n事实上，① 的等号仅当 $x = 0$，② 的等号仅当 $x = 1$，两者无法同时取到，故严格不等式成立：$e^x - \\ln x > 2$（$x > 0$）。',
    tags: ['放缩法', '切线放缩', '不等式链', '严格不等式'],
    source: '自主命题',
  },
  ...
  // ===== 难度 3：数列求和型不等式 =====
  {
    ...
    difficulty: 3,
    type: 'proof',
    content: '(1) 证明：当 $x > 0$ 时，$\\frac{x}{1+x} < \\ln(1+x) < x$。\n(2) 利用 (1) 证明：对任意正整数 $n$（$n \\in \\mathbb{N}^*$），有\n$$\\ln(n+1) < \\sum_{k=1}^{n} \\frac{1}{k} < 1 + \\ln n.$$',
    answer: '证明见解答。',
    solution: '(1) 右半部分 $\\ln(1+x) < x$（$x > 0$）：由 $\\ln(1+x) \\le x$ 且 $x > 0$ 时等号不成立（等号仅 $x = 0$），故 $\\ln(1+x) < x$。\n左半部分 $\\frac{x}{1+x} < \\ln(1+x)$：令 $G(x) = \\ln(1+x) - \\frac{x}{1+x}$（$x > 0$）。$G\'(x) = \\frac{1}{1+x} - \\frac{(1+x) - x}{(1+x)^2} = \\frac{1}{1+x} - \\frac{1}{(1+x)^2} = \\frac{x}{(1+x)^2} > 0$（$x > 0$）。又 $G(0) = 0$，故 $G(x) > 0$（$x > 0$），即 $\\frac{x}{1+x} < \\ln(1+x)$。\n\n(2) 在 (1) 的不等式中取 $x = \\frac{1}{k}$（$k = 1, 2, \\dots, n$）：\n$$\\frac{1/k}{1 + 1/k} < \\ln\\Bigl(1 + \\frac{1}{k}\\Bigr) < \\frac{1}{k}$$\n化简左端：$\\frac{1/k}{1 + 1/k} = \\frac{1}{k+1}$。中段：$\\ln\\frac{k+1}{k} = \\ln(k+1) - \\ln k$。\n于是对每个 $k$ 有：\n$$\\frac{1}{k+1} < \\ln(k+1) - \\ln k < \\frac{1}{k}$$\n\n先用右半边不等式：$\\ln(k+1) - \\ln k < \\frac{1}{k}$。对 $k = 1, 2, \\dots, n$ 累加求和：\n$$\\sum_{k=1}^{n} [\\ln(k+1) - \\ln k] < \\sum_{k=1}^{n} \\frac{1}{k}$$\n左边裂项相消得 $\\ln(n+1) - \\ln 1 = \\ln(n+1)$，故 $\\ln(n+1) < \\sum_{k=1}^{n} \\frac{1}{k}$。\n\n再用左半边不等式：$\\frac{1}{k+1} < \\ln(k+1) - \\ln k$。对 $k = 1, 2, \\dots, n-1$ 累加求和：\n$$\\sum_{k=1}^{n-1} \\frac{1}{k+1} < \\sum_{k=1}^{n-1} [\\ln(k+1) - \\ln k] = \\ln n$$\n即 $\\sum_{j=2}^{n} \\frac{1}{j} < \\ln n$。两边加 $1$：$1 + \\sum_{j=2}^{n} \\frac{1}{j} < 1 + \\ln n$，即 $\\sum_{k=1}^{n} \\frac{1}{k} < 1 + \\ln n$。\n\n综合即得 $\\ln(n+1) < \\sum_{k=1}^{n} \\frac{1}{k} < 1 + \\ln n$。',
    tags: ['数列不等式', '累加求和', '裂项相消', '调和级数'],
    source: '自主命题',
  },
];
