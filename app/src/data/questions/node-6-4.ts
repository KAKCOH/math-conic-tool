...
const questions6_4: (Question | ChoiceQuestion)[] = [
  ...
  // ===== 难度 3：放缩法处理零点问题 =====
  {
    ...
    difficulty: 3,
    type: 'calculation',
    content: '已知函数 $f(x) = \\ln x + x - 2$。\n\n(1) 证明：$f(x)$ 有唯一零点，记作 $x_0$，并判断 $x_0$ 所在的整数区间；\n(2) 利用不等式 $\\ln x \\le x - 1$（$x > 0$），证明 $x_0 \\ge \\frac{3}{2}$；\n(3) 利用不等式 $\\ln x \\ge 1 - \\frac{1}{x}$（$x > 0$），证明 $x_0 \\le \\frac{1 + \\sqrt{5}}{2}$。',
    answer: '证明见解答。$x_0$ 满足 $\\frac{3}{2} \\le x_0 \\le \\frac{1 + \\sqrt{5}}{2}$。',
    solution: '(1) $f\'(x) = \\frac{1}{x} + 1 > 0$ 对一切 $x > 0$ 成立，故 $f(x)$ 在 $(0, +\\infty)$ 上严格单调递增，至多有一个零点。\n$f(1) = \\ln 1 + 1 - 2 = -1 < 0$，$f(2) = \\ln 2 + 2 - 2 = \\ln 2 > 0$。由零点存在定理，$f(x)$ 在 $(1, 2)$ 有唯一零点 $x_0$。\n\n(2) 由 $f(x_0) = 0$ 得 $\\ln x_0 + x_0 = 2$。\n利用 $\\ln x_0 \\le x_0 - 1$（切线放缩，等号仅当 $x_0 = 1$，但 $x_0 > 1$ 故严格小于）：\n$2 = \\ln x_0 + x_0 < (x_0 - 1) + x_0 = 2x_0 - 1$。\n即 $2x_0 - 1 > 2$，解得 $x_0 > \\frac{3}{2}$。结合严格单调性，$x_0 \\ge \\frac{3}{2}$（取等不成立，但作为下界估计可写非严格）。\n\n(3) 先证辅助不等式 $\\ln x \\ge 1 - \\frac{1}{x}$（$x > 0$）。令 $h(x) = \\ln x - 1 + \\frac{1}{x}$，$h\'(x) = \\frac{1}{x} - \\frac{1}{x^2} = \\frac{x-1}{x^2}$。$0 < x < 1$ 时 $h\' < 0$，$x > 1$ 时 $h\' > 0$，$h(x) \\ge h(1) = 0$。故 $\\ln x \\ge 1 - \\frac{1}{x}$ 对所有 $x > 0$ 成立。\n\n由 $f(x_0) = 0$ 及上述不等式：\n$2 = \\ln x_0 + x_0 \\ge \\Bigl(1 - \\frac{1}{x_0}\\Bigr) + x_0 = x_0 + 1 - \\frac{1}{x_0}$。\n两边乘 $x_0 > 0$：$2x_0 \\ge x_0^2 + x_0 - 1$。\n整理：$x_0^2 - x_0 - 1 \\le 0$。\n解二次不等式：$x_0 \\in \\Bigl[\\frac{1 - \\sqrt{5}}{2}, \\frac{1 + \\sqrt{5}}{2}\\Bigr]$。\n结合 $x_0 > 1$，得 $x_0 \\le \\frac{1 + \\sqrt{5}}{2}$。\n\n综合 (2)(3) 得 $\\frac{3}{2} \\le x_0 \\le \\frac{1 + \\sqrt{5}}{2}$。',
    tags: ['放缩法', '零点估计', '切线放缩', '上下界'],
    source: '自主命题',
  },
];
